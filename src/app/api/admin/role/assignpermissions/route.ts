...
export async function POST(request: Request) {
  console.log('API Route: /api/admin/role/assignpermissions called');
  // 从Cookie获取token
  ...
  const apiUrl = `${config.baseUrl}/sys/role/${requestData.roleId}/assign-menus`;
  ...
  try {
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({ menuIds: requestData.menuIds })
    });
    
  // 获取原始响应数据
    const responseData = await response.json();
    console.log("这是分配角色权限的后端API返回的日志输出:");
    console.log("请求url:", apiUrl);
    console.log("请求token:", token);
    console.log("请求参数 roleId:", requestData.roleId);
    console.log("请求参数 menuIds:", requestData.menuIds);
