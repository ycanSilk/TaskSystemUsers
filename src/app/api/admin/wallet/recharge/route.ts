...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // 验证请求参数
    if (!requestData.targetUserId || requestData.amount <= 0 || !requestData.remark) {
      return NextResponse.json<ApiResponse>({
        success: false,
        message: '参数验证失败: targetUserId、amount、remark为必填项，且amount必须大于0'
      }, { status: 400 });
    }
    ...
    // 构建外部API请求
    const apiUrl = `${config.baseUrl}${config.endpoints.wallet.recharge}`;
